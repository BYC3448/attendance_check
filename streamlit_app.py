...
view_date = current_date if selected_date == "오늘 날짜" else selected_date
...
for idx, student in enumerate(students):
    ...
    if cols[4].button("삭제", key=f"del_{sid}"):
        ...
        st.warning(f"{name} 학생이 삭제되었습니다. 새로고침(F5) 해주세요.")
...
with st.form("add_student"):
    ...
    if submitted and new_name.strip():
        ...
        save_students(students)
        st.success(f"학생 '{new_name}' 이(가) 추가되었습니다. 새로고침(F5) 해주세요.")
